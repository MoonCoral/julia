...
If `A` is a matrix, `dims=1` will sort rows, `dims=2` will sort columns.
...
spellings for `dims=1` and `dims=2` respectively.
...
julia> sortslices([7 3 5; -1 6 4; 9 -2 8], dims=:rows)
3×3 Array{Int64,2}:
 -1   6  4
  7   3  5
  9  -2  8
...
passing the 2x2 slices in each dimension to the comparison function. Note that
while there is no default order on higher-dimensional slices, you may use the
`by` or `lt` keyword argument to specify such an order.
...
and then arrange the result back into the array in column-major order. If
`dims` is `(2,1)` instead, the same slices will be taken, but the result order
will be row-major instead.
...
```
"""
function sortslices(A::AbstractArray; dims, kws...)
    if isa(dims, Symbol)
        dims = dims == :rows ? 1 :
               dims == :cols ? 2 :
