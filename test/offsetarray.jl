...
@test sortslices(A, dims=:rows) == OffsetArray(sortslices(parent(A), dims=:rows), A.offsets)
@test sortslices(A, dims=:cols) == OffsetArray(sortslices(parent(A), dims=:cols), A.offsets)
